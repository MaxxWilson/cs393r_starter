...
#include "low_csm_map.h"
#include <algorithm>    // std::max
namespace low_csm_map{

    LowCSMMap::LowCSMMap() : xy_raster_map::XYRasterMap(){
     ...
     */
    void LowCSMMap::GenerateMapFromNewScan(const std::vector<Eigen::Vector2f> &cloud){
        ClearMap();
    ...
    void LowCSMMap::SetLogLikelihoodAtPosition(const double x, const double y, double likelihood){
    ...
    double LowCSMMap::GetLogLikelihoodAtPosition(const double x, const double y) const{
        uint64_t xIdx = GetIndexFromDist(x);
        uint64_t yIdx = GetIndexFromDist(y);
        if(xIdx >= prob_map.size() || yIdx >= prob_map[0].size()) throw std::out_of_range("Out of boundaries");
    ...
    double LowCSMMap::GetNormalizedLikelihoodAtPosition(const double x, const double y) const{
        return GetStandardLikelihoodAtPosition(x, y)/ConvertLogToStandard(max_likelihood);
    }

    double LowCSMMap::GetStandardLikelihoodAtPosition(const double x, const double y) const{
        return ConvertLogToStandard(GetLogLikelihoodAtPosition(x, y));
    }

    double LowCSMMap::ConvertLogToStandard(const double log_likelihood) const{
            ...
            for(int y = 0; y < row_num; y++){
                ...
                for(int i = x * res_ratio; i <= (x + 1) * res_ratio; i++) {
                    for(int j = y * res_ratio; j <= (y + 1) * res_ratio; j++) {
                        try {
                            if(i < 0 || i >= high_res_map.GetRowNum() || j < 0 || j >= high_res_map.GetColNum()) continue;
                            prob_map[x][y] = std::max(prob_map[x][y], high_res_map.GetValueAtIdx(i, j));
                        } catch(...) {
